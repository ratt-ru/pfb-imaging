...
import pyscilog
pyscilog.init('pfb')
log = pyscilog.get_logger('TAIVAS')
...
defaults = {}
for key in schema.taivas["inputs"].keys():
    defaults[key] = schema.taivas["inputs"][key]["default"]
...
def taivas(**kw):
    ...
    pyscilog.log_to_file(f'taivas_{timestamp}.log')
