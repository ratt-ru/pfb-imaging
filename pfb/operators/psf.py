...
def _psf_convolve_band_impl(x, psfhat, beam,
                            nthreads=None,
                            padding=None,
                            unpad_x=None,
                            unpad_y=None,
...
def _psf_convolve_band(x, psfhat, beam, psfopts):
    return _psf_convolve_band_impl(x, psfhat, beam, **psfopts)

def psf_convolve_band(x, psfhat, beam, psfopts):
    if not isinstance(x, da.Array):
        x = da.from_array(x, chunks=(1, -1, -1), name=False)
    if beam is None:
        bout = None
        ...
        bout = ('nb', 'nx', 'ny')
    return da.blockwise(_psf_convolve_band, ('nb', 'nx', 'ny'),
                        x, ('nb', 'nx', 'ny'),
                        psfhat, ('nb', 'nx', 'ny'),
                        beam, bout,
                        psfopts, None,
