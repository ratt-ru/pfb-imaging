...
def generate_transient_spectra(times, freqs, transient_params):
    ...
    time_profile = generate_time_profile(times, time_params["peak_time"], time_params["duration"], time_params["shape"])
        ...
        for n in range(n_periods):
            ...
            if t_start + t_shift <= total_duration:
                shifted_profile = generate_time_profile(
                    times, time_params["peak_time"] + t_shift, time_params["duration"], time_params["shape"]
                )
    ...
    freq_profile = generate_frequency_profile(
        freqs, freq_params["peak_flux"], freq_params["reference_freq"], freq_params["spectral_index"]
    )
