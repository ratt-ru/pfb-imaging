...
from pfb_imaging.operators.gridder import compute_residual, image_data_products
from pfb_imaging.operators.hessian import _hessian_slice
from pfb_imaging.operators.psi import psi_band_maker
from pfb_imaging.opt.pcg import pcg
from pfb_imaging.utils.misc import set_image_size, taperf
...
class band_actor(object):
    def __init__(self, xds_list, opts, bandid, cache_path, max_freq, uv_max):
        ...
        dsl = xds_from_list(xds_list, drop_vars=["VIS", "BEAM"], nthreads=self.nthreads)
        ...
        self.xhat = empty_noncritical((self.nx_psf, self.nyo2), dtype=self.complex_type)
        self.xpad = empty_noncritical((self.nx_psf, self.ny_psf), dtype=self.real_type)
        self.xout = empty_noncritical((self.nx, self.ny), dtype=self.real_type)
    ...
    def set_image_data_products(self, model, k, dof=None, from_cache=False):
        ...
        if from_cache:
            ...
            data_vars = {"UPDATE": (("x", "y"), self.update)}
            self.attrs["niters"] = k
            ...
            self.dds = xds_from_list([self.cache_path], drop_vars=drop_vars)[0]
    ...
    def hess_direct(self, x, mode="forward", sigma=None):
        ...
        r2c(self.xpad, out=self.xhat, forward=True, inorm=0, nthreads=self.nthreads)
        if mode == "forward":
