...
class ISTA(ForwardBackward):
    ...
    def __init__(self, lmbda, gamma, max_iter, precond, step_size=None, tol=1e-12, log_iter=10, return_metrics=False):
        super().__init__(
            max_iter=max_iter, step_size=step_size, tol=tol, log_iter=log_iter, return_metrics=return_metrics
        )
    ...
    def _obj_fun(self, x, y):
        ...
        $$
        """
        val = (1 / (2 * self.gamma)) * (x - y).reshape(1, -1).conjugate().flatten() @ (self.precond.dot(x - y)).reshape(
            -1, 1
        ) + self.lmbda * np.sum(np.abs(x))
        return val.squeeze()
