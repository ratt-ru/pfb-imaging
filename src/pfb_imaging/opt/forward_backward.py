...
# A general abstract base class for the Forward backward algorithm
# which works for arbitrary grad, prox
...
class ForwardBackward:
    ...
    def _get_metrics(self, x, x_prev, y):
        ...
        return {"rel_var": rel_var, "obj_fun": obj_fun}
    ...
    def forward(self, y):
        ...
        # Initialize a dictionary object to store metrics
        # (run time, objective function, relative variation, rms, ...)
        metrics = {}
        obj_fun = self._obj_fun(x, y)
        rel_var = float("inf")
        ...
        while it < self.max_iter and self._convergence_criteria(obj_fun, rel_var):
            ...
            if it % self.log_iter:
                ...
                rel_var, obj_fun = (
                    current_metrics["rel_var"],
                    current_metrics["obj_fun"],
                )  # at least the relative variation and the objective function should be computed.
