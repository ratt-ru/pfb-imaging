...
def nnls(
    ...
    for i in range(maxit):
        fprime = partial(value_and_grad, dirty=residual, psfo=psfo, mask=mask, beam=beam)

        x = fista(np.zeros_like(model), beta, fprime, prox, tol=ftol, maxit=fmaxit, verbosity=fverbose)
        ...
        if outfile is not None:
            ...
            save_fits(outfile + str(i + 1) + "_NNLS_model_mfs.fits", model_mfs, hdr_mfs)

            save_fits(outfile + str(i + 1) + "_NNLS_model.fits", model, hdr)

            save_fits(outfile + str(i + 1) + "_NNLS_residual_mfs.fits", residual_mfs, hdr_mfs)
