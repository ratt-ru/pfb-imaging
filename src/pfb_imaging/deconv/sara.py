...
def sara(
    ...
    if len(residual.shape) > 3:
        log.error_and_raise("Residual must have shape (nband, nx, ny)", ValueError)
    ...
    if beam_image is None:
        ...
        try:
            ...
            log.error_and_raise("Beam has incorrect shape", ValueError)
    ...
    if mask is None:
        ...
        try:
            ...
            log.error_and_raise("Mask has incorrect shape", ValueError)
    ...
    beta, betavec = power_method(hessb, residual.shape, tol=pmtol, maxit=pmmaxit, verbosity=pmverbose)
    ...
    for i in range(0, maxit):
        ...
        beta, betavec = power_method(hessb, residual.shape, b0=betavec, tol=pmtol, maxit=pmmaxit, verbosity=pmverbose)

        log.info("Iter %i: peak residual = %f, rms = %f, eps = %f" % (i + 1, rmax, rms, eps))
        ...
        if outfile is not None:
            ...
            save_fits(outfile + str(i + 1) + "_model_mfs.fits", model_mfs, hdr_mfs)
            ...
            save_fits(outfile + str(i + 1) + "_residual_mfs.fits", residual_mfs, hdr_mfs)

            save_fits(outfile + str(i + 1) + "_residual.fits", residual * wsum, hdr)
