...
def _smoovie(**kw):
    ...
    try:
        ...
        log.error_and_raise(f"There must be a dataset at {fds_store.url}", ValueError)
    ...
    def plot_frame(ds):
        ...
        rms = ds.rms[0, 0]
        im1 = ax.imshow(resid, vmin=-opts.min_frac * rms, vmax=opts.max_frac * rms, cmap=opts.cmap)
    ...
    if opts.animate_axis == "time":
        ...
        log.error_and_raise(f"Can't animate axis {opts.animate_axis}", NotImplementedError)
