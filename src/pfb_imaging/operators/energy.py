...
from pfb_imaging.utils.stokes import corr_to_stokes, stokes_to_corr
...
def pol_energy_approx(ID, psfhat, x):
    ...
    # Convolution
    Bhat = jnp.fft.fft2(B, s=(nx_psf, ny_psf), axes=(0, 1), norm="backward")
    Bconv = jnp.fft.ifft2(Bhat * psfh, s=(nx_psf, ny_psf), axes=(0, 1), norm="backward")[0:nx, 0:ny, :]
    ...
    return jnp.vdot(B, Bconv).real - jnp.vdot(B, Id).real * 2
    # jnp.sum(a*a) +
    # jnp.sum(b*b) +
    # jnp.sum(c*c) +
    # jnp.sum(d*d))
...
if __name__ == "__main__":
    ...
    psf = vis2dirty(
        uvw=uvw,
        freq=freq,
        vis=2 * np.ones((m, 1), dtype=np.complex128),
        npix_x=2 * n,
        npix_y=2 * n,
        pixsize_x=cell_n,
        pixsize_y=cell_n,
    ...
    # Convolution
    Bhat = jnp.fft.fft2(B, s=(2 * n, 2 * n), axes=(0, 1), norm="backward")
    Bconv = jnp.fft.ifft2(Bhat * psfhat, s=(2 * n, 2 * n), axes=(0, 1), norm="backward")[0:n, 0:n, :] / 2
