...
def psf_convolve_cube(
    ...
    r2c(xpad, axes=(1, 2), nthreads=nthreads, forward=True, inorm=0, out=xhat)
    xhat *= psfhat
...
def psf_convolve_fscube(
    ...
    xpad[...] = 0.0
    xpad[:, :, 0:nx, 0:ny] = x
    r2c(xpad, axes=(-2, -1), nthreads=nthreads, forward=True, inorm=0, out=xhat)
    xhat *= psfhat
    c2r(
        xhat,
        axes=(-2, -1),
