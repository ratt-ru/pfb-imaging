...
def restore(
    ...
    """
    Restore clean components to residual images.
    ...
    Args:
        ...
        outputs: Output products to generate
        overwrite: Allow overwriting fits files
        gausspar: Gaussian parameters for restoration
        inflate_factor: Inflate the intrinsic resolution
        drop_bands: List of bands to discard
        host_address: Distributed client address
        nworkers: Number of worker processes
        nthreads: Number of threads per worker
        direct_to_workers: Connect direct to workers
        log_level: Logging level
        output_filename: Basename of output
        log_directory: Directory for logs
        product: Stokes products to produce
        fits_output_folder: Path for fits files
        fits_mfs: Output MFS fits files
        fits_cubes: Output fits cubes
    """
    # Lazy import heavy dependencies
    from pfb.workers.restore import restore as restore_impl
