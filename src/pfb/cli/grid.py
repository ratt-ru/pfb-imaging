...
def grid(
    ...
    Args:
        xds: Optional explicit path to xds. Set using output-filename and suffix by default.
        suffix: Can be used to specify a custom name for the image space data products
        concat_row: Concatenate datasets by row
        overwrite: Allow overwriting of image space data products. Specify suffix to create a new data set.
        transfer_model_from: Name of dataset to use for model initialisation
        use_best_model: If this flag is set MODEL_BEST will be used as the model unless transfer-model-from is specified. By default MODEL will be used as the model.
        robustness: Robustness factor for Briggs weighting. None means natural
        dirty: Compute the dirty image
        psf: Compute the PSF
        residual: Compute the residual (only if model is present)
        noise: Compute noise map by sampling from weights
        beam: Interpolate average beam pattern
        psf_oversize: Size of PSF relative to dirty image
        weight: Compute effectve image space weights
