...
def hci(
    ...
    Args:
        ...
        scans: List of SCAN_NUMBERS to image. Defaults to all. Input as comma separated list 0,2 if running from CLI
        ddids: List of DATA_DESC_ID's to images. Defaults to all. Input as comma separated list 0,2 if running from CLI
        fields: List of FIELD_ID's to image. Defaults to all. Input as comma separated list 0,2 if running from CLI
        freq_range: Frequency range to image in Hz. Specify as a string with colon delimiter eg. '1e9:1.1e9'
        overwrite: Allow overwrite of output xds
        transfer_model_from: Name of dataset to use for model initialisation
        data_column: Data column to image. Must be the same across MSs. Simple arithmetic is supported eg. 'CORRECTED_DATA-MODEL_DATA'. When gains are present this column will be corrected.
        model_column: Model column to subtract from corrected data column. Useful to avoid high time res degridding.
        weight_column: Column containing natural weights. Must be the same across MSs
        sigma_column: Column containing standard devations. Will be used to initialise natural weights if detected. Must be the same across MSs
        flag_column: Column containing data flags. Must be the same across MSs
        gain_table: Path to Quartical gain table containing NET gains. There must be a table for each MS and glob(ms) and glob(gt) should match up when running from CLI.
        integrations_per_image: Number of time integrations per image. Default (-1, 0, None) -> dataset per scan.
        channels_per_image: Number of channels per image for gridding resolution. Default of (-1, 0, None) implies a single dataset per spw.
        precision: Gridding precision
        beam_model: Path to beam model as an xarray dataset backed by zarr
        field_of_view: Field of view in degrees
        super_resolution_factor: Will over-sample Nyquist by this factor at max frequency
        cell_size: Cell size in arc-seconds
        nx: Number of x pixels
        ny: Number of y pixels
        psf_relative_size: Relative size of the PSF in pixels. A value of 1.0 means the PSF will be the same size as the image. The default is to make the PSF just big enough to extract the PSF parameters. If the natural gradient is required the PSF needs to ideally be twice the size of the image.
        robustness: Robustness factor for Briggs weighting. None means natural
        target: This can be predefined celestial objects known to astropy or a string in the format 'HH:MM:SS,DD:MM:SS' (note the , delimiter)
        l2_reweight_dof: The degrees of freedom parameter for L2 reweighting. The default (None) means no reweighting. A sensible value for this parameter depends on the level of RFI in the data. Small values (eg. 2) result in aggressive reweighting and should be avoided if the model is still incomplete.
        progressbar: Display progress. Use --no-progressbar to deactivate.
        output_format: zarr or fits output
        eta: Value to add to Hessian to make it invertible. Smaller values tend to fit more noise and make the inversion less stable.
        psf_out: Whether to produce output PSF's or not.
        weight_grid_out: Whether to produce an image of the weighted grid or not.
        natural_grad: Compute naural gradient
        check_ants: Check that ANTENNA1 and ANTENNA2 tables are consistent with the ANTENNA table.
        inject_transients: YAML file containing transients to inject into the data.
        filter_counts_level: Set minimum counts in the uniform weighting grid to the median divided by this value. This is useful to avoid artificially up-weighting nearly empty uv-cells.
        min_padding: Minimum padding to be applied during gridding.
        phase_dir: Rephase visibilities to this phase center. Should be a string in the format 'HH:MM:SS,DD:MM:SS' (note the , delimiter)
        stack: Stack everything into a single xarray dataset
        epsilon: Gridder accuracy
        do_wgridding: Perform w-correction via improved w-stacking
        double_accum: Accumulate onto grid using double precision. Only has an affect when using single precision.
        ...
        log_level: 
        cg_tol: Tolreance of conjugate gradient algorithm
        cg_maxit: Maximum iterations for conjugate gradient algorithm
        cg_minit: Minimum iterations for conjugate gradient algorithm
        cg_verbose: Verbosity of conjugate gradient algorithm. Set to > 1 for debugging, 0 for silence.
        cg_report_freq: Report frequency of conjugate gradient algorithm.
        backtrack: Ensure residual decreases at every iteration.
    """
