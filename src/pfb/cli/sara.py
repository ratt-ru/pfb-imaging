...
def sara(
    ...
    Args:
        ...
        bases: Wavelet bases to use. Give as comma separated str eg.
        nlevels: Wavelet decomposition level
        l1_reweight_from: L1 reweighting will kick in either at convergence or after this many iterations. Set to a negative value to disbale L1 reweighting.
        hess_norm: Spectral norm of hessian approximation
        hess_approx: Which Hessian approximation to use. Set to wgt for vis space approximation, psf for zero-padded image space approximation and direct for direct inversion.
        rmsfactor: By default will threshold by rmsfactor*rms at every iteration
        eta: Will use eta*wsum to regularise the inversion of the Hessian approximation.
        gamma: Step size of update
        alpha: Controls how aggressively the l1reweighting is applied. Larger values correspond to more agressive reweighting.
        nbasisf: Number of basis functions to use while fitting the frequency axis. Default is to use the number of non-null imaging bands i.e. interpolation.
        positivity: How to apply positivity constraint 0 -> no positivity, 1 -> normal positivity constraint 2 -> strong positivity i.e. all pixels in a band > 0
        niter: Number of iterations.
        nthreads: Total number of threads to use. Defaults to half the total number available.
        tol: Tolerance at which to terminate algorithm. Will stop when norm(x-xp)/norm(x) < tol
        diverge_count: Will terminate the algorithm if the rms increases this many times. Set to larger than niter to disable this check.
        skip_model: Skip model creation i.e. use with do-res to do-res only
        rms_outside_model: Mask residual where model is non-zero when computing rms. This is not recommended for largely non-empty fields.
        init_factor: Reduce the regularisation strength by this fraction at the outset.
        verbosity: Set to larger than 1 to report timings during residual computation
        ...
        pd_tol: Tolreance of primal dual algorithm
        pd_maxit: Maximum iterations for primal dual algorithm
        pd_verbose: Verbosity of primal dual algorithm. Set to > 1 for debugging, 0 for silence
        pd_report_freq: Report frequency of primal dual algorithm
        pm_tol: Tolreance of power method
        pm_maxit: Maximum iterations for power method
        pm_verbose: Verbosity of primal power method. Set to > 1 for debugging, 0 for silence
        pm_report_freq: Report frequency of power method algorithm
